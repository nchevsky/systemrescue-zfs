...
mkinitcpio_comp_opts="--memlimit=10G --threads=0 --verbose"
...
case ${arch} in
    x86_64)
        ...
        mkinitcpio_comp_opts="${mkinitcpio_comp_opts} --lzma2=preset=9e,dict=128MiB"
        ;;
    i686)
        efiarch="i386-efi"
...
make_boot_extra() {
    ...
    cp ${work_dir}/${arch}/airootfs/usr/share/licenses/spdx/GPL-2.0-only.txt ${work_dir}/iso/${install_dir}/boot/memtest.COPYING
